...
if (isset($_GET['archivo'])) { //Verifica ruta del archivo
	...
	if (file_exists($xmlFilePath)){ //Verifica existencia del archivo
		...
		if(!empty($xmlContent)){
			...
			$url = $urlToSend; // Cambia la URL a la que corresponde
			...
			curl_setopt($ch, CURLOPT_USERPWD, $curlUser . ':' . $curlPass);
			...
			if (curl_errno($ch)) {
				...
				if (strpos(curl_error($ch), "Could not resolve host") !== false) {
					...
					$logString = "Error: ".date("Y-m-d\TH:i:s")." -- Archivo: ".$xmlFilePath." -- ".curl_error($ch)."\n\n";
				}
				...
				if (strpos($result, "Unauthorized") !== false){
				        echo "<h3>Se ha producido un error al enviar la informacion, consulte con el administrador</h3>";
                                        $logString = "Error: ".date("Y-m-d\TH:i:s")." -- Archivo: ".$xmlFilePath." -- Acceso no autorizado\n\n";
				} else if(strpos($result, "Forbidden") !== false){
	                                echo "<h3>Se ha producido un error al enviar la informacion, consulte con el administrador</h3>";
                                        $logString = "Error: ".date("Y-m-d\TH:i:s")." -- Archivo: ".$xmlFilePath." -- Acceso denegado\n\n";
				} else {
					...
					if (strpos($xmlString, 'OK/OK') ) {
						echo "<h3> La validacion del resultado fue correcta</h3>";
        	                                ...
        	                                $logString = "Fecha: ".date("Y-m-d\TH:i:s")." -- Archivo: ".$xmlFilePath."\nRespuesta: ".$result."\n";
						...
						$xmlString = $result ;
						$logString = "Error: ".date("Y-m-d\TH:i:s")." -- Archivo: ".$xmlFilePath." -- ".$result."\n\n";
					}
			...
			echo '</div>';
			$logString = "Error: ".date("Y-m-d\TH:i:s")." -- Archivo: ".$xmlFilePath." -- Archivo XML vacio\n\n";
		}
    } else {
		echo '<div class="d-flex flex-column align-items-center">';
		...
		echo	'<a href="index.php" class="btn btn-primary">Volver al inicio</a>';
		echo '</div>';
		$logString = "Error: ".date("Y-m-d\TH:i:s")." -- Archivo: ".$xmlFilePath." -- Archivo XML inexistente\n\n";
    }
} else {
	echo '<div class="d-flex flex-column align-items-center">';
	...
	echo	'<a href="index.php" class="btn btn-primary">Volver al inicio</a>';
	echo '</div>';
	$logString = "Error: ".date("Y-m-d\TH:i:s")." -- Archivo: Ruta no especificada\n\n";
}
